...
function convertColumn(position) {
...
function lightCell(position) {
  let row = position.substring(1, 3) - 1;
  let col = convertColumn(position);
...
function isShip(position) {
  if(lightCell(position) == "v") {
    return true;
  }
  return false;
}
...
function allRocks() {
  ...
  for(let col = 0; col < GRID.length; col++) { //navigate per col not rows
    for(let row = 0; row < GRID[col].length; row++) {
      rowLetter = String.fromCharCode(65 + row); //translate number to letter
      position = rowLetter + (col + 1); //add 1 to col because it starts on 1
...
function howDangerous(position) {
  if(isRock(position)) return 100;
  if(isCurrent(position)) return 50;
  return 0;
}
...
function percentageReport() {
  let rockPercentage = ((allRocks().length * 100) / totalCells()).toFixed(2);
  let currentPercentage = ((allCurrents().length * 100) / totalCells()).toFixed(2);
...
function safetyReport() {
  let gridSafety = GRID.map(row => {return row.map(cell => howDangerousValue(cell));
  });
...
function calcDistance(pos1, pos2) {
  let row1 = pos1.substring(1, 3) - 1;
  let col1 = pos1.charCodeAt(0) - 65;
  let row2 = pos2.substring(1, 3) - 1;
  let col2 = pos2.charCodeAt(0) - 65;

  return ((Math.sqrt(Math.pow((row2 - row1), 2) + Math.pow((col2 - col1), 2)))).toFixed(2);
}
